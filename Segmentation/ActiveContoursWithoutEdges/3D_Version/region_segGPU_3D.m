...
%------------------------------------------------------------------------
function seg = region_segGPU(Iloc,init_mask_loc,max_its,alpha,display)
  ...
  %-- make mask from SDF
  seg = gather(phi<=0); %-- Get mask from levelset
...
function D = sussman(D, dt)
  ...
  dD = parallel.gpu.GPUArray.zeros(size(D));
  D_neg_ind = find(D < 0);
  D_pos_ind = find(D > 0);

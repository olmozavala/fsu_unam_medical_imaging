...
function newim = ridgefilter3D(im, orient1, orient2, freq, kx, ky,kz, showfilter)
    ...
    for k = 1:length(unfreq)
        ...
        for o = 1:180/angleInc
            for i=1:size(reffilter,3)
                ...
                filter1{i,k,o} = imrotate(reffilter2D,-(o*angleInc+90),'bilinear','crop');
            end
            for i=1:size(reffilter,1)
                reffilter2D=squeeze(reffilter(i,:,:));
                filter2{i,k,o} = imrotate(reffilter2D,-(o*angleInc+90),'bilinear','crop');
            end  
        end
    end
    ...
    for k = 1:length(finalind)
        ...
        t = sze(filterindex);   
        newim(r,c,s) = sum(sum(sum(im(r-t:r+t, c-t:c+t,s-t:s+t).*filter{filterindex,orientindex(r,c,s)})));
    end

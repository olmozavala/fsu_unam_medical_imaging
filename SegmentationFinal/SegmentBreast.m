function [ mask,OutputCoordL, OutputCoordR ] = SegmentBreast( imgData )
...
OutputCoordL = PPrimaL;
OutputCoordR = PPrimaR;
...
% for x = 1:size(BodyFatMasksROI,3); 
...
for x = 1:nimages/2; 
    ...
    medianV = median(xq);
    if(medianV>(numRows/2)+ numRows*0.1 || medianV<(numRows/2)- numRows*0.1)
        error(x) = 99999;
    else
        currentError =  immse(yq,J);
